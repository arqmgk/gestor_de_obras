...
export const register = async (req, res, next) => {
  try {
    const { nombre, email, password, empresa, rol } = req.body

    // Validar rol
    const rolValido = ['arquitecto', 'capataz'].includes(rol) ? rol : 'arquitecto'
    ...
    const empresaResult = await pool.query(
      `INSERT INTO empresas (nombre) VALUES ($1) RETURNING *`,
      [empresa]
    )
    ...
    const userResult = await pool.query(
      ...
      [nombre, email, hashedPassword, rolValido, empresaId]
    )
